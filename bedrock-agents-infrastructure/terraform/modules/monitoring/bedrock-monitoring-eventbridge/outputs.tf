...
output "event_bus_console_url" {
  ...
  value       = "https://console.aws.amazon.com/events/home?region=${data.aws_region.current.name}#/eventbus/${local.event_bus_name}"
}

output "rules_console_url" {
  description = "URL to view EventBridge rules in AWS Console"
  value       = "https://console.aws.amazon.com/events/home?region=${data.aws_region.current.name}#/rules"
}
