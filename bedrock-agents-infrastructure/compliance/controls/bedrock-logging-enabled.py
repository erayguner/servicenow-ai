    ...
    try:
        ...
        if not model_logging_config:
            ...
            if not cloudwatch_config:
                violations.append(
                    "CloudWatch logging is not enabled for model invocations"
                )
            else:
                ...
                if not log_group_name:
                    ...
                    if not log_group_encrypted:
                        violations.append(
                            f"CloudWatch log group {log_group_name} is not encrypted"
                        )

                if not role_arn:
                    violations.append(
                        "IAM role for CloudWatch logging is not specified"
                    )
            ...
            if s3_config:
                ...
                if bucket_name:
                    ...
                    try:
                        ...
                        violations.append(
                            f"S3 logging bucket {bucket_name} is not encrypted"
                        )

            # Check text data delivery
            text_data_delivery = logging_config.get("textDataDeliveryEnabled")
            if (
                rule_parameters.get("RequireTextDataDelivery", "false").lower()
                == "true"
            ):
                if not text_data_delivery:
                    violations.append(
                        "Text data delivery is not enabled (prompts and completions not logged)"
                    )
...
if __name__ == "__main__":
    test_event = {
        ...
        "ruleParameters": json.dumps({"RequireTextDataDelivery": "true"}),
