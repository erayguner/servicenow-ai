    ...
    try:
        ...
        for policy in attached_policies.get('AttachedPolicies', []):
            ...
            if policy_arn in dangerous_managed_policies:
                violations.append(f"Overly permissive managed policy attached: {policy_arn}")
            ...
            for statement in policy_doc.get('Statement', []):
                if statement.get('Effect') == 'Allow':
                    ...
                    for action in actions:
                        if action == '*':
                            violations.append(f"Inline policy '{policy_name}' grants all actions (*)")
                        elif action.endswith(':*'):
                            # Service-level wildcard (e.g., s3:*) - warn if broad
                            service = action.split(':')[0]
                            if service in ['iam', 'sts', 'kms', 'organizations']:
                                violations.append(f"Inline policy '{policy_name}' grants broad permissions: {action}")
                    ...
                    for resource in resources:
                        if resource == '*':
                            ...
                            read_only = all(
                                any(action.startswith(prefix) for prefix in ['Get', 'List', 'Describe', 'bedrock:Invoke', 'bedrock:Retrieve'])
                                for action in actions
                            )

                            if not read_only:
                                violations.append(f"Inline policy '{policy_name}' allows write actions on all resources (*)")
