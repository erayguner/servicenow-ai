...
def evaluate_bedrock_vpc_endpoints(rule_parameters):
    ...
    try:
        ...
        enforce_private_dns = rule_parameters.get('EnforcePrivateDNS', 'true').lower() == 'true'
        ...
        if not bedrock_endpoints:
            violations.append("No Bedrock VPC endpoints found - RESTRICTED data access must use VPC endpoints")
        else:
            for endpoint in bedrock_endpoints:
                ...
                if state != 'available':
                    violations.append(f"VPC endpoint {endpoint_id} is not in 'available' state (current: {state})")

                # Check if in required VPC
                if required_vpc_id and vpc_id != required_vpc_id:
                    violations.append(f"VPC endpoint {endpoint_id} is not in required VPC {required_vpc_id} (found in {vpc_id})")

                # Check private DNS
                if enforce_private_dns and not private_dns_enabled:
                    violations.append(f"VPC endpoint {endpoint_id} does not have private DNS enabled")

                # Check security groups
                security_groups = endpoint.get('Groups', [])
                if not security_groups:
                    violations.append(f"VPC endpoint {endpoint_id} has no security groups attached")
                else:
                    ...
                    for sg in security_groups:
                        ...
                        if sg_violations:
                            violations.extend([f"VPC endpoint {endpoint_id} SG {sg_id}: {v}" for v in sg_violations])

                # Check subnets (should be private subnets)
                subnet_ids = endpoint.get('SubnetIds', [])
                if not subnet_ids:
                    violations.append(f"VPC endpoint {endpoint_id} has no subnets configured")
                else:
                    for subnet_id in subnet_ids:
                        if not is_private_subnet(subnet_id):
                            violations.append(f"VPC endpoint {endpoint_id} subnet {subnet_id} is not a private subnet (has route to IGW)")
    ...
    try:
        ...
        for rule in ingress_rules:
            ...
            for ip_range in ip_ranges:
                ...
                if cidr == '0.0.0.0/0':
                    violations.append("Ingress rule allows access from 0.0.0.0/0 (internet)")

            ipv6_ranges = rule.get('Ipv6Ranges', [])
            for ipv6_range in ipv6_ranges:
                cidr = ipv6_range.get('CidrIpv6', '')
                if cidr == '::/0':
                    violations.append("Ingress rule allows access from ::/0 (internet IPv6)")
